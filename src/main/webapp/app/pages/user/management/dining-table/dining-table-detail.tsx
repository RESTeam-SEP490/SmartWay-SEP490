...
export const DiningTableDetail = ({ diningTable, onUpdate }: { diningTable: IDiningTable; onUpdate: any }) => {
  ...
  return (
    <>
      <Tabs defaultValue={1}>
        <Tabs.TabPane tab={translate('menuItem.infoTabs.information')} key={1} className="p-4 details max-w-[calc((100vw-32px)*4/5-66px)]">
          <div className="">
        ...
        <Tabs.TabPane tab={translate('diningTable.infoTabs.sold')} key={2}></Tabs.TabPane>
        <Tabs.TabPane tab={translate('diningTable.infoTabs.ingredients')} key={3}></Tabs.TabPane>
      </Tabs>
      <DiningTableDelete diningTables={[diningTable]} handleClose={() => setIsShowDialog(false)} isOpen={isShowDialog} />
    </>
  );
};
