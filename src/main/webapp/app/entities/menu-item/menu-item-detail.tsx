import React from 'react';
import { Translate, translate } from 'react-jhipster';

import { CheckSquareFilled, CloseSquareFilled, DeleteFilled, EditFilled, LockFilled, PlusOutlined } from '@ant-design/icons';
import { Button, Form, Image, Input, InputNumber, Tabs, Typography } from 'antd';
import TextArea from 'antd/es/input/TextArea';
import { DEFAULT_FORM_ITEM_LAYOUT, currencyFormatter } from 'app/app.constant';
import { IMenuItem } from '../../shared/model/menu-item.model';

export const MenuItemDetail = ({ menuItem }: { menuItem: IMenuItem }) => {
  return (
    <>
      <Tabs className="p-2" defaultValue={1}>
        <Tabs.TabPane tab={translate('menuItem.infoTabs.information')} key={1} className="p-2 details">
          <div className="">
            <Typography.Title level={4}>{menuItem.name}</Typography.Title>
          </div>

          <div className="flex">
            <div>
              <Image
                src={menuItem.imageUrl}
                width={200}
                height={200}
                className="shadow-md overflow-hidden rounded-md border-spacing-2 border-solid border-blue-300"
                fallback="data:image/png;base64,"
              />
            </div>
            <Form {...DEFAULT_FORM_ITEM_LAYOUT} colon className="ml-6 flex-grow">
              <Form.Item className="!mb-0" label={translate('menuItem.code.label')}>
                <Input bordered={false} value={menuItem.code} />
              </Form.Item>
              <Form.Item className="!mb-0" label={translate('menuItem.category.label')}>
                <Input bordered={false} value={menuItem.menuItemCategory.name} />
              </Form.Item>
              <Form.Item className="!mb-0" label={translate('menuItem.basePrice.label')}>
                <InputNumber bordered={false} value={menuItem.basePrice} formatter={currencyFormatter} />
              </Form.Item>
              <Form.Item className="!mb-0" label={translate('menuItem.sellPrice.label')}>
                <InputNumber bordered={false} value={menuItem.sellPrice} formatter={currencyFormatter} />
              </Form.Item>
            </Form>
            <Form colon layout="vertical" className="ml-6 flex-grow">
              <Form.Item colon label={translate('global.table.description')}>
                <TextArea autoSize style={{ resize: 'none', height: 'max-content' }} bordered={false} />
              </Form.Item>
              <div className="pb-4">
                <div className="flex gap-2">
                  {menuItem.isAllowSale ? (
                    <>
                      <CheckSquareFilled rev={''} className="text-green-500 text-lg" />
                      <Translate contentKey="menuItem.isAllowSale.label" />
                    </>
                  ) : (
                    <>
                      <CloseSquareFilled rev={''} className="text-red-500 text-lg" />
                      <Translate contentKey="menuItem.isAllowSale.label" />
                    </>
                  )}
                </div>
                <div className="flex gap-2">
                  {menuItem.isExtraItem ? (
                    <>
                      <CheckSquareFilled rev={''} className="text-green-500 text-lg" />
                      <Translate contentKey="menuItem.isExtraItem.label" />
                    </>
                  ) : (
                    <>
                      <CloseSquareFilled rev={''} className="text-red-500 text-lg" />
                      <Translate contentKey="menuItem.isExtraItem.label" />
                    </>
                  )}
                </div>
              </div>
            </Form>
          </div>
          <div className="flex mt-4 justify-end gap-2">
            <Button type="primary">
              <EditFilled rev={''} />
              <Translate contentKey="entity.action.edit" />
            </Button>
            <Button danger type="primary">
              <LockFilled rev={''} />
              <Translate contentKey="entity.action.stopSale" />
            </Button>
            <Button type="primary" danger>
              <DeleteFilled rev={''} />
              <Translate contentKey="entity.action.delete" />
            </Button>
          </div>
        </Tabs.TabPane>
        <Tabs.TabPane tab={translate('menuItem.infoTabs.extra')} key={2}></Tabs.TabPane>
        <Tabs.TabPane tab={translate('menuItem.infoTabs.ingredients')} key={3}></Tabs.TabPane>
      </Tabs>
    </>
  );
};

export default MenuItemDetail;
